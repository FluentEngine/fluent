...
#define LOG_FATAL( fmt, ... )                                                  \
	 ...
	 * @param[in] maxFileSize The maximum number of bytes to write to any one
	 * file
	 ...
	 */
	void
	logger_setLevel( logger_LogLevel level );
	 ...
	 */
	int
	logger_isEnabled( logger_LogLevel level );

	/**
	 * Flush automatically.
	 * Auto flush is off in default.
	 *
	 * @param[in] interval A fulsh interval in milliseconds. Switch off if 0 or
	 * a negative integer.
	 */
	void
	...
	/**
	 * Log a message.
	 * Make sure to call one of the following initialize functions before
	 * starting logging.
	 ...
	 */
	void
	logger_log( logger_LogLevel level,
	            const char*     file,
	            int             line,
	            const char*     fmt,
	            ... );
