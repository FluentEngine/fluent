...
static volatile logger_LogLevel s_logLevel = LogLevel_INFO;
...
void logger_setLevel(logger_LogLevel level)
{
    s_logLevel = level;
}

logger_LogLevel logger_getLevel(void)
{
    return s_logLevel;
}

int logger_isEnabled(logger_LogLevel level)
{
...
void logger_log(logger_LogLevel level, const char* file, int line, const char* fmt, ...)
{
	struct timeval now;
