...
typedef struct _D3D12_SIGNATURE_PARAMETER_DESC
{
	...
	D3D_NAME SystemValueType; // A predefined system value, or
	                          // D3D_NAME_UNDEFINED if not applicable
...
typedef struct _D3D12_SHADER_TYPE_DESC
{
	...
	UINT Rows;    // Number of rows (for matrices, 1 for other numeric, 0 if not
	              // applicable)
...
typedef struct _D3D12_FUNCTION_DESC
{
	...
	INT FunctionParameterCount; // Number of logical parameters in the function
...
typedef interface ID3D12ShaderReflectionVariable
...
typedef interface ID3D12FunctionParameterReflection
...
DECLARE_INTERFACE_( ID3D12ShaderReflection, IUnknown )
{
	...
	( THIS_ _In_ REFIID iid, _Out_ LPVOID * ppv ) PURE;
	STDMETHOD_( ULONG, AddRef )( THIS ) PURE;
	STDMETHOD_( ULONG, Release )( THIS ) PURE;

	STDMETHOD( GetDesc )( THIS_ _Out_ D3D12_SHADER_DESC * pDesc ) PURE;

	STDMETHOD_( ID3D12ShaderReflectionConstantBuffer *,
	...
	( THIS_ _In_ UINT ParameterIndex,
	...
	STDMETHOD( GetMinFeatureLevel )
	( THIS_ _Out_ enum D3D_FEATURE_LEVEL * pLevel ) PURE;

	STDMETHOD_( UINT, GetThreadGroupSize )
	( THIS_ _Out_opt_ UINT * pSizeX,
...
DECLARE_INTERFACE_( ID3D12LibraryReflection, IUnknown )
{
	...
	( THIS_ _In_ INT FunctionIndex ) PURE;
};
...
DECLARE_INTERFACE( ID3D12FunctionReflection )
{
	...
	STDMETHOD_( ID3D12ShaderReflectionConstantBuffer *,
	            GetConstantBufferByIndex )
	( THIS_ _In_ UINT BufferIndex ) PURE;
	STDMETHOD_( ID3D12ShaderReflectionConstantBuffer *,
	            GetConstantBufferByName )
	( THIS_ _In_ LPCSTR Name ) PURE;
	...
	( THIS_ _In_ INT ParameterIndex ) PURE;
};
