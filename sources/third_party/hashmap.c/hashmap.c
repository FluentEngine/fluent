...
{
	return ( struct bucket * ) ( ( ( char * ) map->buckets ) +
	                             ( map->bucketsz * index ) );
}

static void *
bucket_item( struct bucket *entry )
{
...
hashmap_new( size_t   elsize,
             ...
             void *udata )
{
	return hashmap_new_with_allocator( ( _malloc ? _malloc : malloc ),
...
void
hashmap_clear( struct hashmap *map, bool update_cap )
{
	map->count = 0;
	free_elements( map );
	if ( update_cap )
...
{
	struct hashmap *map2 = hashmap_new( map->elsize,
	                                    new_cap,
	                                    map->seed1,
	                                    map->seed1,
	                                    map->hash,
...
void *
hashmap_set( struct hashmap *map, const void *item )
{
	if ( !item )
	{
		panic( "item is null" );
	...
	for ( ;; )
	{
		...
		if ( entry->hash == bucket->hash && map->compare( bucket_item( entry ),
		                                                  bucket_item( bucket ),
		                                                  map->udata ) == 0 )
		{
...
bool
hashmap_oom( struct hashmap *map )
{
	return map->oom;
}
...
//-----------------------------------------------------------------------------
static void
MM86128( const void *key, const int len, uint32_t seed, void *out )
{
...
{
	return hashmap_murmur( *( char ** ) item,
	                       strlen( *( char ** ) item ),
	                       seed0,
	                       seed1 );
}
