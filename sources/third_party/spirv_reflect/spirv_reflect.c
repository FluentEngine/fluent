...
static uint32_t
RoundUp( uint32_t value, uint32_t multiple )
{
...
static int
SortCompareUint32( const void* a, const void* b )
{
...
//
static size_t
DedupSortedUint32( uint32_t* arr, size_t size )
{
	if ( size == 0 )
	{
		return 0;
...
static bool
SearchSortedUint32( const uint32_t* arr, size_t size, uint32_t target )
{
	size_t lo = 0;
	size_t hi = size;
	while ( lo < hi )
...
IntersectSortedUint32( const uint32_t* p_arr0,
                       size_t          arr0_size,
...
static bool
InRange( const SpvReflectPrvParser* p_parser, uint32_t index )
{
	bool in_range = false;
...
static SpvReflectResult
ReadStr( SpvReflectPrvParser* p_parser,
         uint32_t             word_offset,
         uint32_t             word_index,
         uint32_t             word_count,
         uint32_t*            p_buf_size,
...
{
	memcpy( p_numeric_traits,
	        &p_type->traits.numeric,
...
{
	memcpy( p_array_traits,
	        &p_type->traits.array,
	        sizeof( p_type->traits.array ) );
}

static SpvReflectPrvNode*
FindNode( SpvReflectPrvParser* p_parser, uint32_t result_id )
{
...
static SpvReflectTypeDescription*
FindType( SpvReflectShaderModule* p_module, uint32_t type_id )
{
...
static SpvReflectResult
CreateParser( size_t size, void* p_code, SpvReflectPrvParser* p_parser )
{
	if ( p_code == NULL )
	{
...
static void
DestroyParser( SpvReflectPrvParser* p_parser )
{
...
static SpvReflectResult
ParseNodes( SpvReflectPrvParser* p_parser )
{
	assert( IsNotNull( p_parser ) );
	assert( IsNotNull( p_parser->spirv_code ) );
	...
	p_parser->nodes =
	    ( SpvReflectPrvNode* ) calloc( p_parser->node_count,
	...
	for ( uint32_t i = 0; i < node_count; ++i )
	{
		...
		p_parser->nodes[ i ].decorations.binding.value =
		    ( uint32_t ) INVALID_VALUE;
		p_parser->nodes[ i ].decorations.location.value =
		    ( uint32_t ) INVALID_VALUE;
		p_parser->nodes[ i ].decorations.offset.value =
		    ( uint32_t ) INVALID_VALUE;
		p_parser->nodes[ i ].decorations.uav_counter_buffer.value =
		    ( uint32_t ) INVALID_VALUE;
		p_parser->nodes[ i ].decorations.built_in =
		    ( SpvBuiltIn ) INVALID_VALUE;
	}
	...
	while ( spirv_word_index < p_parser->spirv_word_count )
	{
		...
		{
			const char* p_source = ( const char* ) ( p_parser->spirv_code +
			                                         p_node->word_offset + 1 );

			const size_t source_len = strlen( p_source );
			const size_t embedded_source_len =
			    strlen( p_parser->source_embedded );
			char* p_continued_source =
			    ( char* ) calloc( source_len + embedded_source_len + 1,
			                      sizeof( char* ) );
...
#ifdef _WIN32
			strcpy_s( p_continued_source,
			          embedded_source_len + 1,
		...
		case SpvOpMemberName:
		{
			...
			p_node->name =
			    ( const char* ) ( p_parser->spirv_code + name_start );
		}
		break;
		...
		case SpvOpTypeImage:
		{
			...
			CHECKED_READU32( p_parser,
			                 p_node->word_offset + 5,
			                 p_node->image_traits.arrayed );
			CHECKED_READU32( p_parser,
			                 p_node->word_offset + 6,
			                 p_node->image_traits.ms );
			CHECKED_READU32( p_parser,
			                 p_node->word_offset + 7,
			                 p_node->image_traits.sampled );
			CHECKED_READU32( p_parser,
			                 p_node->word_offset + 8,
		...
		case SpvOpAccessChain:
		{
			...
			//
			// SPIRV_ACCESS_CHAIN_INDEX_OFFSET (4) is the number of words up
			...
			if ( p_access_chain->index_count > 0 )
			{
				      ...
				      ++index_index )
				{
					...
					SpvReflectPrvNode* p_index_value_node =
					    FindNode( p_parser, index_id );
					     ...
					     ( p_index_value_node->op == SpvOpConstant ) )
					{
						...
						CHECKED_READU32( p_parser,
						                 p_index_value_node->word_offset + 3,
						                 index_value );
		...
		case SpvOpFunction:
		{
			CHECKED_READU32( p_parser,
			                 p_node->word_offset + 2,
			                 p_node->result_id );
			// Count function definitions, not function declarations.  To
			// determine the difference, set an in-function variable, and then
			// if an OpLabel is reached before the end of the function increment
			// the function count.
		...
		case SpvOpLabel:
		{
			if ( function_node != ( uint32_t ) INVALID_VALUE )
			{
				SpvReflectPrvNode* p_func_node =
				    &( p_parser->nodes[ function_node ] );
				CHECKED_READU32( p_parser,
				                 p_func_node->word_offset + 2,
...
static SpvReflectResult
ParseStrings( SpvReflectPrvParser* p_parser )
{
	assert( IsNotNull( p_parser ) );
	assert( IsNotNull( p_parser->spirv_code ) );
	     ...
	     IsNotNull( p_parser->nodes ) )
	{
		...
		p_parser->strings =
		    ( SpvReflectPrvString* ) calloc( p_parser->string_count,
		...
		for ( size_t i = 0; i < p_parser->node_count; ++i )
		{
			...
			// Result id
			SpvReflectPrvString* p_string =
			    &( p_parser->strings[ string_index ] );
			CHECKED_READU32( p_parser,
			                 p_node->word_offset + 1,
			                 p_string->result_id );
			...
			p_string->string =
			    ( const char* ) ( p_parser->spirv_code + string_start );
...
static SpvReflectResult
ParseSource( SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module )
{
	assert( IsNotNull( p_parser ) );
	assert( IsNotNull( p_parser->spirv_code ) );
               ...
               size_t                 first_label_index )
{
	...
	{
		p_func->callees =
		    ( uint32_t* ) calloc( p_func->callee_count,
	...
	{
		p_func->accessed_ptrs =
		    ( uint32_t* ) calloc( p_func->accessed_ptr_count,
	...
	if ( p_func->accessed_ptr_count > 0 )
	{
		qsort( p_func->accessed_ptrs,
		       p_func->accessed_ptr_count,
		       sizeof( *( p_func->accessed_ptrs ) ),
		       SortCompareUint32 );
	}
	p_func->accessed_ptr_count =
	    ( uint32_t ) DedupSortedUint32( p_func->accessed_ptrs,
	                                    p_func->accessed_ptr_count );
...
static int
SortCompareFunctions( const void* a, const void* b )
{
...
static SpvReflectResult
ParseFunctions( SpvReflectPrvParser* p_parser )
{
	assert( IsNotNull( p_parser ) );
	assert( IsNotNull( p_parser->spirv_code ) );
	assert( IsNotNull( p_parser->nodes ) );
	     ...
	     IsNotNull( p_parser->nodes ) )
	{
		...
		for ( size_t i = 0; i < p_parser->node_count; ++i )
		{
			...
			// Intentionally reuse i to avoid iterating over these nodes more
			// than once
			...
			SpvReflectResult result =
			    ParseFunction( p_parser, p_node, p_function, i );
			if ( result != SPV_REFLECT_RESULT_SUCCESS )
			{
				return result;
			}
		...
		qsort( p_parser->functions,
		       p_parser->function_count,
		       sizeof( *( p_parser->functions ) ),
		       SortCompareFunctions );

		// Once they're sorted, link the functions with pointers to improve
...
static SpvReflectResult
ParseMemberCounts( SpvReflectPrvParser* p_parser )
{
	assert( IsNotNull( p_parser ) );
	assert( IsNotNull( p_parser->spirv_code ) );
	assert( IsNotNull( p_parser->nodes ) );
	     ...
	     IsNotNull( p_parser->nodes ) )
	{
		...
		for ( uint32_t i = 0; i < p_parser->node_count; ++i )
		{
			...
			p_node->member_names =
			    ( const char** ) calloc( p_node->member_count,
...
static SpvReflectResult
ParseNames( SpvReflectPrvParser* p_parser )
{
	assert( IsNotNull( p_parser ) );
	assert( IsNotNull( p_parser->spirv_code ) );
	assert( IsNotNull( p_parser->nodes ) );
	     ...
	     IsNotNull( p_parser->nodes ) )
	{
		...
		{
			SpvReflectPrvNode* p_node = &( p_parser->nodes[ i ] );
			if ( ( p_node->op != SpvOpName ) &&
			     ( p_node->op != SpvOpMemberName ) )
			{
				continue;
			}
			...
			if ( p_node->op == SpvOpMemberName )
			{
				...
				CHECKED_READU32( p_parser,
				                 ...
				                 member_index );
				pp_target_name =
				    &( p_target_node->member_names[ member_index ] );
			}
...
ParseDecorations( SpvReflectPrvParser* p_parser )
{
	for ( uint32_t i = 0; i < p_parser->node_count; ++i )
	{
		...
		if ( ( ( uint32_t ) p_node->op != ( uint32_t ) SpvOpDecorate ) &&
		     ...
		     ( ( uint32_t ) p_node->op !=
		       ( uint32_t ) SpvReflectOpDecorateId ) &&
		     ( ( uint32_t ) p_node->op !=
		       ( uint32_t ) SpvReflectOpDecorateStringGOOGLE ) &&
		     ( ( uint32_t ) p_node->op !=
		       ( uint32_t ) SpvReflectOpMemberDecorateStringGOOGLE ) )
		{
			continue;
		}
		...
		CHECKED_READU32( p_parser,
		                 p_node->word_offset + member_offset + 2,
		                 decoration );
		...
		if ( IsNull( p_target_node ) )
		{
			...
			{
				// Many OPs can be decorated that we don't care about. Ignore
				// those. See
				// https://github.com/KhronosGroup/SPIRV-Reflect/issues/134
				continue;
			}
			return SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ID_REFERENCE;
		}
		...
		if ( p_node->op == SpvOpMemberDecorate )
		{
			...
			p_target_decorations =
			    &( p_target_node->member_decorations[ member_index ] );
		}
		...
		case SpvDecorationDescriptorSet:
		{
			...
			CHECKED_READU32( p_parser,
			                 word_offset,
			                 p_target_decorations->set.value );
		...
		case SpvDecorationInputAttachmentIndex:
		{
			...
			p_target_decorations->input_attachment_index.word_offset =
			    word_offset;
		}
		break;
                      ...
                      uint32_t**              pp_uniforms )
{
	...
	*pp_uniforms =
	    ( uint32_t* ) calloc( *p_uniform_count, sizeof( **pp_uniforms ) );
...
static SpvReflectResult
ParseType( SpvReflectPrvParser*       p_parser,
           ...
           SpvReflectTypeDescription* p_type )
{
	...
	if ( p_node->member_count > 0 )
	{
		...
		if ( IsNotNull( p_type->members ) )
		{
			...
			for ( size_t i = 0; i < p_type->members->member_count; ++i )
			{
				...
				p_member_type->storage_class =
				    ( SpvStorageClass ) INVALID_VALUE;
			}
		}
		else
		{
	...
	if ( result == SPV_REFLECT_RESULT_SUCCESS )
	{
		// Since the parse descends on type information, these will get
		// overwritten if not guarded against assignment. Only assign if the id
		// is invalid.
		...
		// Issue and fix here:
		// https://github.com/chaoticbob/SPIRV-Reflect/issues/64
		...
		case SpvOpTypeVector:
		{
			...
			SpvReflectPrvNode* p_next_node =
			    FindNode( p_parser, component_type_id );
			if ( IsNotNull( p_next_node ) )
			{
				result =
				    ParseType( p_parser, p_next_node, NULL, p_module, p_type );
			}
			else
			{
		...
		case SpvOpTypeMatrix:
		{
			...
			SpvReflectPrvNode* p_next_node =
			    FindNode( p_parser, column_type_id );
			if ( IsNotNull( p_next_node ) )
			{
				result =
				    ParseType( p_parser, p_next_node, NULL, p_module, p_type );
			...
			p_type->traits.numeric.matrix.stride =
			    p_node->decorations.matrix_stride;
		...
		case SpvOpTypeImage:
		{
			...
			IF_READU32( result,
			            ...
			            sampled_type_id );
			SpvReflectPrvNode* p_next_node =
			    FindNode( p_parser, sampled_type_id );
			if ( IsNotNull( p_next_node ) )
			{
				result =
				    ParseType( p_parser, p_next_node, NULL, p_module, p_type );
		...
		case SpvOpTypeSampledImage:
		{
			...
			IF_READU32( result,
			            ...
			            image_type_id );
			SpvReflectPrvNode* p_next_node =
			    FindNode( p_parser, image_type_id );
			if ( IsNotNull( p_next_node ) )
			{
				result =
				    ParseType( p_parser, p_next_node, NULL, p_module, p_type );
		...
		case SpvOpTypeArray:
		{
			...
			if ( result == SPV_REFLECT_RESULT_SUCCESS )
			{
				...
				//       OpMemberDecorate, even if the array is apart of a
				//       struct.
				p_type->traits.array.stride = p_node->decorations.array_stride;
				// Get length for current dimension
				SpvReflectPrvNode* p_length_node =
				    FindNode( p_parser, length_id );
				if ( IsNotNull( p_length_node ) )
				{
					...
					else
					{
						...
						IF_READU32( result,
						            p_parser,
						            p_length_node->word_offset + 3,
						            length );
						if ( result == SPV_REFLECT_RESULT_SUCCESS )
						{
							// Write the array dim and increment the count and
							// offset
							p_type->traits.array.dims[ dim_index ] = length;
							p_type->traits.array
							    .spec_constant_op_ids[ dim_index ] = 0xFFFFFFFF;
							p_type->traits.array.dims_count += 1;
						}
						else
						{
					...
					SpvReflectPrvNode* p_next_node =
					    FindNode( p_parser, element_type_id );
					if ( IsNotNull( p_next_node ) )
					{
						result = ParseType( p_parser,
						                    p_next_node,
						                    NULL,
						                    p_module,
						                    p_type );
		...
		case SpvOpTypeStruct:
		{
			...
			for ( ; word_index < p_node->word_count;
			      ++word_index, ++member_index )
			{
				uint32_t member_id = ( uint32_t ) INVALID_VALUE;
				IF_READU32( result,
				            p_parser,
				            p_node->word_offset + word_index,
				            member_id );
				// Find member node
				SpvReflectPrvNode* p_member_node =
				    FindNode( p_parser, member_id );
				...
				p_member_type->struct_member_name =
				    p_node->member_names[ member_index ];
			}
		}
		break;
...
static SpvReflectResult
ParseTypes( SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module )
{
	...
	p_module->_internal->type_descriptions =
	    ( SpvReflectTypeDescription* ) calloc(
...
static int
SortCompareDescriptorBinding( const void* a, const void* b )
{
                         ...
                         SpvReflectShaderModule* p_module )
{
	      ...
	      ++descriptor_index )
	{
		...
		p_descriptor->descriptor_type =
		    ( SpvReflectDescriptorType ) INVALID_VALUE;
	...
	for ( size_t i = 0; i < p_parser->node_count; ++i )
	{
		...
		SpvReflectTypeDescription* p_type =
		    FindType( p_module, p_node->type_id );
		if ( IsNull( p_type ) )
		{
			return SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ID_REFERENCE;
		}
		// If the type is a pointer, resolve it. We need to retain the storage
		// class from the pointer so that we can use it to deduce deescriptor
		// types.
		...
		p_descriptor->uav_counter_id =
		    p_node->decorations.uav_counter_buffer.value;
		p_descriptor->type_description = p_type;
		p_descriptor->decoration_flags =
		    ApplyDecorations( &p_node->decorations );

		// If this is in the StorageBuffer storage class, it's for sure a
		// storage buffer descriptor. We need to handle this case earlier
		// because in SPIR-V there are two ways to indicate a storage buffer: 1)
		// Uniform storage class + BufferBlock decoration, or 2) StorageBuffer
		// storage class + Buffer decoration. The 1) way is deprecated since
		// SPIR-V v1.3. But the Buffer decoration is also used together with
		// Uniform storage class to mean uniform buffer.. We'll handle the
		// pre-v1.3 cases in ParseDescriptorType().
		...
		if ( p_type->traits.array.dims_count > 0 )
		{
			...
			for ( uint32_t dim_index = 0;
			      dim_index < p_type->traits.array.dims_count;
			      ++dim_index )
			{
		...
		p_descriptor->word_offset.binding =
		    p_node->decorations.binding.word_offset;
...
static SpvReflectResult
ParseDescriptorType( SpvReflectShaderModule* p_module )
{
	if ( p_module->descriptor_binding_count == 0 )
	{
		return SPV_REFLECT_RESULT_SUCCESS;
...
static SpvReflectResult
ParseDescriptorBlockVariable( SpvReflectPrvParser*       p_parser,
                              SpvReflectShaderModule*    p_module,
                              SpvReflectTypeDescription* p_type,
                              SpvReflectBlockVariable*   p_var )
{
	...
	if ( IsNotNull( p_type->members ) && ( p_type->member_count > 0 ) )
	{
		...
		p_var->members =
		    ( SpvReflectBlockVariable* ) calloc( p_var->member_count,
		        ...
		        p_type_node->op == SpvOpTypeRuntimeArray )
		{
			if ( p_type_node->op == SpvOpTypeArray )
			{
				p_type_node =
				    FindNode( p_parser,
				              p_type_node->array_traits.element_type_id );
			}
			else
			{
		      ...
		      ++member_index )
		{
			...
			{
				SpvReflectResult result =
				    ParseDescriptorBlockVariable( p_parser,
				                                  p_module,
				                                  p_member_type,
				                                  p_member_var );
				if ( result != SPV_REFLECT_RESULT_SUCCESS )
			...
			p_member_var->decoration_flags = ApplyDecorations(
...
static SpvReflectResult
ParseDescriptorBlockVariableSizes( SpvReflectPrvParser*     p_parser,
                                   SpvReflectShaderModule*  p_module,
                                   bool                     is_parent_root,
                                   bool                     is_parent_aos,
                                   bool                     is_parent_rta,
	      ...
	      ++member_index )
	{
		...
		case SpvOpTypeVector:
		{
			uint32_t size =
			    ...
			    ( p_member_type->traits.numeric.scalar.width /
			      SPIRV_BYTE_WIDTH );
		...
		case SpvOpTypeArray:
		{
			...
			{
				SpvReflectResult result =
				    ParseDescriptorBlockVariableSizes( p_parser,
				                                       p_module,
				                                       false,
				                                       true,
				                                       is_parent_rta,
			...
			uint32_t element_count =
			    ( p_member_var->array.dims_count > 0 ? 1 : 0 );
...
static void
MarkSelfAndAllMemberVarsAsUsed( SpvReflectBlockVariable* p_var )
{
...
static SpvReflectResult
ParseDescriptorBlockVariableUsage( SpvReflectPrvParser*      p_parser,
                                   SpvReflectShaderModule*   p_module,
                                   SpvReflectPrvAccessChain* p_access_chain,
                                   ...
                                   SpvReflectBlockVariable*  p_var )
{
	...
	case SpvOpTypeArray:
	{
		// Parse through array's type hierarchy to find the actual/non-array
		// element type
		...
		{
			// Parse current var again with a type override and advanced index
			// index
			SpvReflectResult result =
			    ParseDescriptorBlockVariableUsage( p_parser,
			                                       p_module,
			                                       p_access_chain,
			                                       index_index,
			                                       p_type->op,
                       ...
                       SpvReflectShaderModule* p_module )
{
	      ...
	      ++descriptor_index )
	{
		      ...
		      ++access_chain_index )
		{
			...
			result = ParseDescriptorBlockVariableUsage( p_parser,
			                                            p_module,
			                                            p_access_chain,
			                                            0,
			                                            ( SpvOp ) INVALID_VALUE,
		...
		bool is_parent_rta = ( p_descriptor->descriptor_type ==
		                       SPV_REFLECT_DESCRIPTOR_TYPE_STORAGE_BUFFER );
		result             = ParseDescriptorBlockVariableSizes( p_parser,
                                                    p_module,
                                                    true,
                                                    false,
                                                    is_parent_rta,
...
static SpvReflectResult
ParseInterfaceVariable( SpvReflectPrvParser*            p_parser,
                        ...
                        bool*                           p_has_built_in )
{
	...
	if ( p_type->member_count > 0 )
	{
		...
		for ( uint32_t member_index = 0;
		      member_index < p_type_node->member_count;
		      ++member_index )
		{
			SpvReflectPrvDecorations* p_member_decorations =
			    &p_type_node->member_decorations[ member_index ];
			SpvReflectTypeDescription* p_member_type =
			    &p_type->members[ member_index ];
			SpvReflectInterfaceVariable* p_member_var =
			    &p_var->members[ member_index ];
			SpvReflectResult result =
			    ParseInterfaceVariable( p_parser,
			                            NULL,
			                            p_member_decorations,
			                            p_module,
			                            p_member_type,
                         ...
                         uint32_t*               p_interface_variable_ids )
{
	...
	if ( p_entry->input_variable_count > 0 )
	{
		p_entry->input_variables = ( SpvReflectInterfaceVariable** ) calloc(
		    p_entry->input_variable_count,
		    sizeof( *( p_entry->input_variables ) ) );
	...
	if ( p_entry->output_variable_count > 0 )
	{
		p_entry->output_variables = ( SpvReflectInterfaceVariable** ) calloc(
		    p_entry->output_variable_count,
		    sizeof( *( p_entry->output_variables ) ) );
                           ...
                           uint32_t**              p_push_constants )
{
	...
	*p_push_constants = ( uint32_t* ) calloc( *p_push_constant_count,
	...
	qsort( *p_push_constants,
	       *p_push_constant_count,
	       sizeof( **p_push_constants ),
	       SortCompareUint32 );
	return SPV_REFLECT_RESULT_SUCCESS;
}
                   ...
                   uint32_t               depth )
{
	if ( depth > p_parser->function_count )
	{
		...
		//   "Recursion: The static function-call graph for an entry point must
		//   not
		//    contain cycles."
	...
	{
		SpvReflectResult result = TraverseCallGraph( p_parser,
		                                             p_func->callee_ptrs[ i ],
		                                             p_func_count,
		                                             p_func_ids,
		                                             depth + 1 );
		if ( result != SPV_REFLECT_RESULT_SUCCESS )
                              ...
                              uint32_t*               push_constants )
{
	...
	{
		p_called_functions =
		    ( uint32_t* ) calloc( called_function_count,
	...
	result                = TraverseCallGraph( p_parser,
                                p_func,
	...
	for ( size_t i = 0, j = 0; i < called_function_count; ++i )
	{
		...
		memcpy( &used_variables[ used_variable_count ],
		        ...
		        p_parser->functions[ j ].accessed_ptr_count *
		            sizeof( *used_variables ) );
		used_variable_count += p_parser->functions[ j ].accessed_ptr_count;
	}
	SafeFree( p_called_functions );
	...
	if ( used_variable_count > 0 )
	{
		qsort( used_variables,
		       used_variable_count,
		       sizeof( *used_variables ),
		       SortCompareUint32 );
	}
	used_variable_count =
	    ( uint32_t ) DedupSortedUint32( used_variables, used_variable_count );

	// Do set intersection to find the used uniform and push constants
	size_t used_uniform_count = 0;
	//
	SpvReflectResult result0 = IntersectSortedUint32( used_variables,
	                                                  used_variable_count,
	                                                  uniforms,
	                                                  uniform_count,
	...
	SpvReflectResult result1 =
	    IntersectSortedUint32( used_variables,
	                           used_variable_count,
	                           push_constants,
	                           push_constant_count,
	...
	for ( uint32_t j = 0; j < p_module->descriptor_binding_count; ++j )
	{
		...
		bool found = SearchSortedUint32( used_variables,
		                                 used_variable_count,
		                                 p_binding->spirv_id );
		if ( found )
		{
...
static SpvReflectResult
ParseEntryPoints( SpvReflectPrvParser*    p_parser,
                  SpvReflectShaderModule* p_module )
{
	...
	if ( ( result =
	           EnumerateAllUniforms( p_module, &uniform_count, &uniforms ) ) !=
	...
	if ( ( result = EnumerateAllPushConstants( p_module,
	                                           &push_constant_count,
	                                           &push_constants ) ) !=
	     SPV_REFLECT_RESULT_SUCCESS )
	{
	      ...
	      ++i )
	{
		...
		result                               = ReadStr( p_parser,
                          p_node->word_offset + name_start_word_offset,
                          0,
                          p_node->word_count,
                          &name_length_with_terminator,
                          NULL );
		if ( result != SPV_REFLECT_RESULT_SUCCESS )
		{
			return result;
		...
		uint32_t name_word_count =
		    RoundUp( name_length_with_terminator, SPIRV_WORD_SIZE ) /
		    SPIRV_WORD_SIZE;
		uint32_t interface_variable_count =
		    ( p_node->word_count -
		      ( name_start_word_offset + name_word_count ) );
		uint32_t* p_interface_variables = NULL;
		if ( interface_variable_count > 0 )
		{
			p_interface_variables =
			    ( uint32_t* ) calloc( interface_variable_count,
		      ...
		      ++var_index )
		{
			...
			uint32_t offset =
			    name_start_word_offset + name_word_count + var_index;
			CHECKED_READU32( p_parser,
			                 p_node->word_offset + offset,
			                 var_result_id );
			p_interface_variables[ var_index ] = var_result_id;
		}

		result = ParseInterfaceVariables( p_parser,
		                                  p_module,
		                                  p_entry_point,
...
static SpvReflectResult
ParseExecutionModes( SpvReflectPrvParser*    p_parser,
                     SpvReflectShaderModule* p_module )
{
	assert( IsNotNull( p_parser ) );
	assert( IsNotNull( p_parser->nodes ) );
	     ...
	     IsNotNull( p_parser->nodes ) )
	{
		for ( size_t node_idx = 0; node_idx < p_parser->node_count; ++node_idx )
		{
			...
			for ( size_t entry_point_idx = 0;
			      entry_point_idx < p_module->entry_point_count;
			      ++entry_point_idx )
			{
				if ( p_module->entry_points[ entry_point_idx ].id ==
				     entry_point_id )
				{
			...
			case SpvExecutionModeDepthUnchanged: break;
			...
			case SpvExecutionModeOutputTrianglesNV: break;
			}
			p_entry_point->execution_mode_count++;
		}
		uint32_t* indices = ( uint32_t* ) calloc( p_module->entry_point_count,
		                                          sizeof( indices ) );
		if ( IsNull( indices ) )
		{
			return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
		      ...
		      ++entry_point_idx )
		{
			...
			p_entry_point->execution_modes = ( SpvExecutionMode* ) calloc(
			    p_entry_point->execution_mode_count,
...
static int
SortCompareDescriptorSet( const void* a, const void* b )
{
	const SpvReflectDescriptorSet* p_elem_a =
	    ( const SpvReflectDescriptorSet* ) a;
	const SpvReflectDescriptorSet* p_elem_b =
	    ( const SpvReflectDescriptorSet* ) b;
	int value = ( int ) ( p_elem_a->set ) - ( int ) ( p_elem_b->set );
	// We should never see duplicate descriptor set numbers in a shader; if so,
	// a tiebreaker would be needed here.
	assert( value != 0 );
	return value;
}
	...
	for ( uint32_t i = 0; i < p_module->entry_point_count; ++i )
	{
		...
		for ( uint32_t j = 0; j < p_module->descriptor_set_count; ++j )
		{
			const SpvReflectDescriptorSet* p_set =
			    &p_module->descriptor_sets[ j ];
			for ( uint32_t k = 0; k < p_set->binding_count; ++k )
			{
				bool found =
				    SearchSortedUint32( p_entry->used_uniforms,
...
static SpvReflectResult
ParseDescriptorSets( SpvReflectShaderModule* p_module )
{
...
static SpvReflectResult
CreateShaderModule( uint32_t                flags,
                    size_t                  size,
                    const void*             p_code,
                    SpvReflectShaderModule* p_module )
{
	...
	if ( flags & SPV_REFLECT_MODULE_FLAG_NO_COPY )
	{
		...
		p_module->_internal->spirv_code =
		    ( void* ) p_code; // cast that const away
#endif
		p_module->_internal->spirv_word_count =
		    ( uint32_t ) ( size / SPIRV_WORD_SIZE );
	}
	else
	{
	...
	// Generator
	{
		const uint32_t* p_ptr =
		    ( const uint32_t* ) p_module->_internal->spirv_code;
		p_module->generator =
	...
	if ( result == SPV_REFLECT_RESULT_SUCCESS )
	{
		...
		p_module->descriptor_set_count = 0;
		memset( p_module->descriptor_sets,
		        0,
		        SPV_REFLECT_MAX_DESCRIPTOR_SETS *
		            sizeof( *p_module->descriptor_sets ) );
		// Initialize descriptor set numbers
		for ( uint32_t set_number = 0;
		      set_number < SPV_REFLECT_MAX_DESCRIPTOR_SETS;
		      ++set_number )
		{
			p_module->descriptor_sets[ set_number ].set =
			    ( uint32_t ) INVALID_VALUE;
		}
	}
	if ( result == SPV_REFLECT_RESULT_SUCCESS )
...
SpvReflectResult
spvReflectCreateShaderModule( size_t                  size,
                              const void*             p_code,
                              SpvReflectShaderModule* p_module )
{
...
SpvReflectResult
spvReflectCreateShaderModule2( uint32_t                flags,
                               size_t                  size,
                               const void*             p_code,
                               SpvReflectShaderModule* p_module )
{
...
SpvReflectResult
spvReflectGetShaderModule( size_t                  size,
                           const void*             p_code,
                           SpvReflectShaderModule* p_module )
{
...
static void
SafeFreeTypes( SpvReflectTypeDescription* p_type )
{
	if ( IsNull( p_type ) )
	{
		return;
...
static void
SafeFreeBlockVariables( SpvReflectBlockVariable* p_block )
{
	if ( IsNull( p_block ) )
	{
		return;
...
void
spvReflectDestroyShaderModule( SpvReflectShaderModule* p_module )
{
	if ( IsNull( p_module->_internal ) )
	{
		return;
	...
	if ( ( p_module->_internal->module_flags &
	       SPV_REFLECT_MODULE_FLAG_NO_COPY ) == 0 )
	{
...
uint32_t
spvReflectGetCodeSize( const SpvReflectShaderModule* p_module )
{
	if ( IsNull( p_module ) )
	{
		return 0;
...
const uint32_t*
spvReflectGetCode( const SpvReflectShaderModule* p_module )
{
	if ( IsNull( p_module ) )
	{
		return NULL;
	...
	if ( IsNotNull( pp_bindings ) )
	{
		...
		for ( uint32_t index = 0; index < *p_count; ++index )
		{
			SpvReflectDescriptorBinding* p_bindings =
			    ( SpvReflectDescriptorBinding* ) &p_module
			        ->descriptor_bindings[ index ];
                                   ...
                                   SpvReflectDescriptorSet**     pp_sets )
{
	...
	if ( IsNotNull( pp_sets ) )
	{
		...
		{
			SpvReflectDescriptorSet* p_set =
			    ( SpvReflectDescriptorSet* ) &p_module
    ...
    SpvReflectInterfaceVariable** pp_variables )
{
	...
	if ( IsNotNull( pp_variables ) )
	{
		...
		{
			SpvReflectInterfaceVariable* p_var =
			    &p_entry->interface_variables[ index ];
			pp_variables[ index ] = p_var;
		}
	}
	else
	...
	if ( pp_blocks != NULL )
	{
		...
		for ( uint32_t index = 0; index < *p_count; ++index )
		{
			SpvReflectBlockVariable* p_push_constant_blocks =
			    ( SpvReflectBlockVariable* ) &p_module
			        ->push_constant_blocks[ index ];
...
spvReflectEnumeratePushConstants( const SpvReflectShaderModule* p_module,
                                  ...
                                  SpvReflectBlockVariable**     pp_blocks )
{
	return spvReflectEnumeratePushConstantBlocks( p_module,
	                                              p_count,
	                                              pp_blocks );
}
    ...
    SpvReflectBlockVariable**     pp_blocks )
{
	...
	{
		bool found =
		    SearchSortedUint32( p_entry->used_push_constants,
		...
		if ( found )
		{
			if ( IsNotNull( pp_blocks ) )
			{
				...
				pp_blocks[ count++ ] = ( SpvReflectBlockVariable* ) &p_module
				                           ->push_constant_blocks[ i ];
			}
			else
			{
				++count;
                            ...
                            SpvReflectResult*             p_result )
{
	...
	if ( IsNotNull( p_module ) )
	{
		for ( uint32_t index = 0; index < p_module->descriptor_set_count;
		      ++index )
		{
...
const SpvReflectDescriptorSet*
spvReflectGetEntryPointDescriptorSet( const SpvReflectShaderModule* p_module,
                                      const char*                   entry_point,
                                      uint32_t                      set_number,
                                      SpvReflectResult*             p_result )
{
	...
	if ( IsNotNull( p_module ) )
	{
		...
		for ( uint32_t index = 0; index < p_entry->descriptor_set_count;
		      ++index )
		{
			const SpvReflectDescriptorSet* p_potential =
                                      ...
                                      SpvReflectResult*             p_result )
{
	...
	if ( IsNotNull( p_module ) )
	{
		for ( uint32_t index = 0; index < p_module->input_variable_count;
		      ++index )
		{
    ...
    SpvReflectResult*             p_result )
{
	...
	if ( IsNotNull( p_module ) )
	{
		...
		for ( uint32_t index = 0; index < p_entry->input_variable_count;
		      ++index )
		{
			const SpvReflectInterfaceVariable* p_potential =
			    p_entry->input_variables[ index ];
                                       ...
                                       SpvReflectResult*             p_result )
{
	...
	if ( IsNotNull( p_module ) )
	{
		for ( uint32_t index = 0; index < p_module->output_variable_count;
		      ++index )
		{
			const SpvReflectInterfaceVariable* p_potential =
			    p_module->output_variables[ index ];
...
spvReflectGetOutputVariable( const SpvReflectShaderModule* p_module,
                             ...
                             SpvReflectResult*             p_result )
{
	return spvReflectGetOutputVariableByLocation( p_module,
	                                              location,
	                                              p_result );
}
    ...
    SpvReflectResult*             p_result )
{
	...
	if ( IsNotNull( p_module ) )
	{
		...
		for ( uint32_t index = 0; index < p_entry->output_variable_count;
		      ++index )
		{
			const SpvReflectInterfaceVariable* p_potential =
			    p_entry->output_variables[ index ];
...
{
	return spvReflectChangeDescriptorBindingNumbers( p_module,
	                                                 p_descriptor_binding,
	                                                 new_binding_number,
                                     ...
                                     uint32_t new_set_number )
{
	...
	{
		// The descriptor sets for specific entry points might not be in this
		// set, so just match on set index.
    ...
    uint32_t                           new_location )
{
	...
	for ( uint32_t index = 0; index < p_module->input_variable_count; ++index )
	{
		...
		{
			return ChangeVariableLocation( p_module,
			                               p_module->input_variables[ index ],
			                               new_location );
		}
	}
...
const char*
spvReflectSourceLanguage( SpvSourceLanguage source_lang )
{
	switch ( source_lang )
	{
	...
	case SpvSourceLanguageMax: break;
	}
	return "";
}
