...
radians( f32 degree )
{
	return degree * ( ( f32 ) M_PI / 180.0f );
}

#define LINMATH_H_DEFINE_VEC( n )                                              \
	typedef float       vec##n[ n ];                                           \
	LINMATH_H_FUNC void vec##n##_add( vec##n       r,                          \
	                                  vec##n const a,                          \
	                                  vec##n const b )                         \
	{                                                                          \
		int i;                                                                 \
	...
	LINMATH_H_FUNC void vec##n##_sub( vec##n       r,                          \
	                                  vec##n const a,                          \
	                                  vec##n const b )                         \
	{                                                                          \
		int i;                                                                 \
	...
	LINMATH_H_FUNC void vec##n##_scale( vec##n       r,                        \
	                                    vec##n const v,                        \
	                                    float const  s )                       \
	{                                                                          \
		int i;                                                                 \
	...
	LINMATH_H_FUNC void vec##n##_min( vec##n       r,                          \
	                                  vec##n const a,                          \
	                                  vec##n const b )                         \
	{                                                                          \
		int i;                                                                 \
	...
	LINMATH_H_FUNC void vec##n##_max( vec##n       r,                          \
	                                  vec##n const a,                          \
	                                  vec##n const b )                         \
	{                                                                          \
		int i;                                                                 \
...
LINMATH_H_FUNC void
vec3_mul_cross( vec3 r, vec3 const a, vec3 const b )
{
	r[ 0 ] = a[ 1 ] * b[ 2 ] - a[ 2 ] * b[ 1 ];
	r[ 1 ] = a[ 2 ] * b[ 0 ] - a[ 0 ] * b[ 2 ];
	r[ 2 ] = a[ 0 ] * b[ 1 ] - a[ 1 ] * b[ 0 ];
...
LINMATH_H_FUNC void
vec3_reflect( vec3 r, vec3 const v, vec3 const n )
{
...
LINMATH_H_FUNC void
vec4_mul_cross( vec4 r, vec4 const a, vec4 const b )
{
	r[ 0 ] = a[ 1 ] * b[ 2 ] - a[ 2 ] * b[ 1 ];
	r[ 1 ] = a[ 2 ] * b[ 0 ] - a[ 0 ] * b[ 2 ];
	r[ 2 ] = a[ 0 ] * b[ 1 ] - a[ 1 ] * b[ 0 ];
...
LINMATH_H_FUNC void
vec4_reflect( vec4 r, vec4 const v, vec4 const n )
{
...
LINMATH_H_FUNC void
mat4x4_dup( mat4x4 M, mat4x4 const N )
{
	int i;
	for ( i = 0; i < 4; ++i ) vec4_dup( M[ i ], N[ i ] );
}
LINMATH_H_FUNC void
mat4x4_row( vec4 r, mat4x4 const M, int i )
{
	int k;
	for ( k = 0; k < 4; ++k ) r[ k ] = M[ k ][ i ];
}
LINMATH_H_FUNC void
mat4x4_col( vec4 r, mat4x4 const M, int i )
{
	int k;
	for ( k = 0; k < 4; ++k ) r[ k ] = M[ i ][ k ];
}
LINMATH_H_FUNC void
mat4x4_transpose( mat4x4 M, mat4x4 const N )
{
...
LINMATH_H_FUNC void
mat4x4_add( mat4x4 M, mat4x4 const a, mat4x4 const b )
{
	int i;
	for ( i = 0; i < 4; ++i ) vec4_add( M[ i ], a[ i ], b[ i ] );
}
LINMATH_H_FUNC void
mat4x4_sub( mat4x4 M, mat4x4 const a, mat4x4 const b )
{
	int i;
	for ( i = 0; i < 4; ++i ) vec4_sub( M[ i ], a[ i ], b[ i ] );
}
LINMATH_H_FUNC void
mat4x4_scale( mat4x4 M, mat4x4 const a, float k )
{
	int i;
...
LINMATH_H_FUNC void
mat4x4_mul( mat4x4 M, mat4x4 const a, mat4x4 const b )
{
	mat4x4 temp;
	int    k, r, c;
	for ( c = 0; c < 4; ++c )
...
LINMATH_H_FUNC void
mat4x4_mul_vec4( vec4 r, mat4x4 const M, vec4 const v )
{
	int i, j;
	for ( j = 0; j < 4; ++j )
	{
...
LINMATH_H_FUNC void
mat4x4_rotate_X( mat4x4 Q, mat4x4 const M, float angle )
{
	float  s = sinf( angle );
	float  c = cosf( angle );
...
LINMATH_H_FUNC void
mat4x4_rotate_Y( mat4x4 Q, mat4x4 const M, float angle )
{
	float  s = sinf( angle );
	float  c = cosf( angle );
...
LINMATH_H_FUNC void
mat4x4_rotate_Z( mat4x4 Q, mat4x4 const M, float angle )
{
	float  s = sinf( angle );
	float  c = cosf( angle );
...
LINMATH_H_FUNC void
mat4x4_invert( mat4x4 T, mat4x4 const M )
{
	float s[ 6 ];
	float c[ 6 ];
...
LINMATH_H_FUNC void
mat4x4_orthonormalize( mat4x4 R, mat4x4 const M )
{
	mat4x4_dup( R, M );
	float s = 1.f;
	vec3  h;
...
LINMATH_H_FUNC void
quat_rotate( quat r, float angle, vec3 const axis )
{
	vec3 axis_norm;
...
LINMATH_H_FUNC void
quat_mul_vec3( vec3 r, quat const q, vec3 const v )
{
	/*
...
LINMATH_H_FUNC void
mat4x4_from_quat( mat4x4 M, quat const q )
{
	float a  = q[ 3 ];
	float b  = q[ 0 ];
	float c  = q[ 1 ];
...
LINMATH_H_FUNC void
mat4x4o_mul_quat( mat4x4 R, mat4x4 const M, quat const q )
{
...
LINMATH_H_FUNC void
quat_from_mat4x4( quat q, mat4x4 const M )
{
	float r = 0.f;
	int   i;
                ...
                float        s )
{
	...
	if ( vec2_len( a ) < 1. )
	{
		z_a = sqrtf( 1.0f - vec2_mul_inner( a, a ) );
	}
	else
	{
		vec2_norm( a, a );
	...
	if ( vec2_len( b ) < 1. )
	{
		z_b = sqrtf( 1.0f - vec2_mul_inner( b, b ) );
	}
	else
	{
		vec2_norm( b, b );
