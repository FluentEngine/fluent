...
public:
	static bool
	IsCompressed( DXGIFormat fmt );
	static uint32_t
	MakeFourCC( char ch0, char ch1, char ch2, char ch3 );
	static DXGIFormat
	GetDXGIFormat( const PixelFormat& pf );
	static uint32_t
	GetBitsPerPixel( DXGIFormat fmt );
	...
	Result
	Load( std::vector<uint8_t>&& dds );

	const ImageData*
	GetImageData( uint32_t mipIdx = 0, uint32_t arrayIdx = 0 ) const
	{
...
private:
	...
	GetImageInfo( uint32_t   w,
	              ...
	              uint32_t*  outNumRows );
	bool
	FlipImage( ImageData& imageData );
	bool
	FlipCompressedImage( ImageData& imageData );
	void
	FlipCompressedImageBC1( ImageData& imageData );
	void
	FlipCompressedImageBC2( ImageData& imageData );
	void
	FlipCompressedImageBC3( ImageData& imageData );
	void
	FlipCompressedImageBC4( ImageData& imageData );
	void
	FlipCompressedImageBC5( ImageData& imageData );
...
bool
DDSFile::IsCompressed( DXGIFormat fmt )
{
	switch ( fmt )
	{
	...
	case DXGIFormat::BC7_UNorm_SRGB: return true;
	}
	return false;
}

uint32_t
DDSFile::MakeFourCC( char ch0, char ch1, char ch2, char ch3 )
{
...
DDSFile::DXGIFormat
DDSFile::GetDXGIFormat( const PixelFormat& pf )
{
	if ( pf.m_flags & uint32_t( PixelFormatFlagBits::RGB ) )
	{
		...
		case 32:
			...
			if ( pf.m_RBitMask == 0x00ff0000 && pf.m_GBitMask == 0x0000ff00 &&
			     pf.m_BBitMask == 0x000000ff && pf.m_ABitMask == 0xff000000 )
			{
				return DXGIFormat::B8G8R8A8_UNorm;
			}
			if ( pf.m_RBitMask == 0x00ff0000 && pf.m_GBitMask == 0x0000ff00 &&
			     pf.m_BBitMask == 0x000000ff && pf.m_ABitMask == 0x00000000 )
			{
			...
			if ( pf.m_RBitMask == 0xffffffff && pf.m_GBitMask == 0x00000000 &&
			     pf.m_BBitMask == 0x00000000 && pf.m_ABitMask == 0x00000000 )
			{
	...
	else if ( pf.m_flags & uint32_t( PixelFormatFlagBits::FourCC ) )
	{
		...
		switch ( pf.m_fourCC )
		{
		case 36: return DXGIFormat::R16G16B16A16_UNorm;
		case 110: return DXGIFormat::R16G16B16A16_SNorm;
		case 111: return DXGIFormat::R16_Float;
		case 112: return DXGIFormat::R16G16_Float;
		case 113: return DXGIFormat::R16G16B16A16_Float;
		case 114: return DXGIFormat::R32_Float;
		case 115: return DXGIFormat::R32G32_Float;
		case 116: return DXGIFormat::R32G32B32A32_Float;
		}
	}
...
uint32_t
DDSFile::GetBitsPerPixel( DXGIFormat fmt )
{
	switch ( fmt )
	{
	...
	case DXGIFormat::Y216: return 64;
	...
	case DXGIFormat::YUY2: return 32;

	case DXGIFormat::P010:
	case DXGIFormat::P016: return 24;
	...
	case DXGIFormat::B4G4R4A4_UNorm: return 16;

	case DXGIFormat::NV12:
	case DXGIFormat::YUV420_OPAQUE:
	case DXGIFormat::NV11: return 12;
	...
	case DXGIFormat::P8: return 8;

	case DXGIFormat::R1_UNorm: return 1;
	...
	case DXGIFormat::BC4_Typeless:
	case DXGIFormat::BC4_UNorm:
	case DXGIFormat::BC4_SNorm: return 4;
	...
	case DXGIFormat::BC7_Typeless:
	case DXGIFormat::BC7_UNorm:
	case DXGIFormat::BC7_UNorm_SRGB: return 8;
	default: return 0;
	}
}

Result
DDSFile::Load( const char* filepath )
{
...
Result
DDSFile::Load( std::istream& input )
{
	m_dds.clear();
...
Result
DDSFile::Load( const uint8_t* data, size_t size )
{
...
Result
DDSFile::Load( std::vector<uint8_t>&& dds )
{
	m_dds.clear();
	...
	if ( ( header->m_pixelFormat.m_flags &
	     ...
	     ( MakeFourCC( 'D', 'X', '1', '0' ) ==
	       header->m_pixelFormat.m_fourCC ) )
	{
		if ( ( sizeof( uint32_t ) + sizeof( Header ) +
		       sizeof( HeaderDXT10 ) ) >= dds.size() )
		{
			return Result::ErrorSize;
		}
		dxt10Header = true;
	...
	if ( dxt10Header )
	{
		...
		case DXGIFormat::IA44:
		case DXGIFormat::P8:
		case DXGIFormat::A8P8: return Result::ErrorNotSupported;
		default:
		...
		case TextureDimension::Texture3D:
			...
			break;
		default: return Result::ErrorNotSupported;
		}
...
void
DDSFile::GetImageInfo( uint32_t   w,
                       uint32_t   h,
                       DXGIFormat fmt,
                       uint32_t*  outNumBytes,
                       uint32_t*  outRowBytes,
...
bool
DDSFile::FlipImage( ImageData& imageData )
{
...
bool
DDSFile::FlipCompressedImage( ImageData& imageData )
{
...
void
DDSFile::FlipCompressedImageBC1( ImageData& imageData )
{
...
void
DDSFile::FlipCompressedImageBC2( ImageData& imageData )
{
	uint32_t numXBlocks = ( imageData.m_width + 3 ) / 4;
	uint32_t numYBlocks = ( imageData.m_height + 3 ) / 4;
	if ( imageData.m_height == 1 ) {}
...
void
DDSFile::FlipCompressedImageBC3( ImageData& imageData )
{
	uint32_t numXBlocks = ( imageData.m_width + 3 ) / 4;
	uint32_t numYBlocks = ( imageData.m_height + 3 ) / 4;
	if ( imageData.m_height == 1 ) {}
		...
		for ( uint32_t y = 0; y < ( numYBlocks + 1 ) / 2; y++ )
		{
			...
			for ( uint32_t x = 0; x < numXBlocks; x++ )
			{
				...
				if ( block0 != block1 )
				{
					...
					uint8_t r0[ 6 ];
					r0[ 0 ] =
					    ( block0->m_alphaR4 >> 4 ) | ( block0->m_alphaR5 << 4 );
					r0[ 1 ] =
					    ( block0->m_alphaR5 >> 4 ) | ( block0->m_alphaR3 << 4 );
					r0[ 2 ] =
					    ( block0->m_alphaR3 >> 4 ) | ( block0->m_alphaR4 << 4 );
					r0[ 3 ] =
					    ( block0->m_alphaR1 >> 4 ) | ( block0->m_alphaR2 << 4 );
					r0[ 4 ] =
					    ( block0->m_alphaR2 >> 4 ) | ( block0->m_alphaR0 << 4 );
					r0[ 5 ] =
					    ( block0->m_alphaR0 >> 4 ) | ( block0->m_alphaR1 << 4 );
					uint8_t r1[ 6 ];
					r1[ 0 ] =
					    ( block1->m_alphaR4 >> 4 ) | ( block1->m_alphaR5 << 4 );
					r1[ 1 ] =
					    ( block1->m_alphaR5 >> 4 ) | ( block1->m_alphaR3 << 4 );
					r1[ 2 ] =
					    ( block1->m_alphaR3 >> 4 ) | ( block1->m_alphaR4 << 4 );
					r1[ 3 ] =
					    ( block1->m_alphaR1 >> 4 ) | ( block1->m_alphaR2 << 4 );
					r1[ 4 ] =
					    ( block1->m_alphaR2 >> 4 ) | ( block1->m_alphaR0 << 4 );
					r1[ 5 ] =
					    ( block1->m_alphaR0 >> 4 ) | ( block1->m_alphaR1 << 4 );
...
void
DDSFile::FlipCompressedImageBC4( ImageData& imageData )
{
	uint32_t numXBlocks = ( imageData.m_width + 3 ) / 4;
	uint32_t numYBlocks = ( imageData.m_height + 3 ) / 4;
	if ( imageData.m_height == 1 ) {}
		...
		for ( uint32_t y = 0; y < ( numYBlocks + 1 ) / 2; y++ )
		{
			...
			for ( uint32_t x = 0; x < numXBlocks; x++ )
			{
				...
				if ( block0 != block1 )
				{
					...
					uint8_t r0[ 6 ];
					r0[ 0 ] =
					    ( block0->m_redR4 >> 4 ) | ( block0->m_redR5 << 4 );
					r0[ 1 ] =
					    ( block0->m_redR5 >> 4 ) | ( block0->m_redR3 << 4 );
					r0[ 2 ] =
					    ( block0->m_redR3 >> 4 ) | ( block0->m_redR4 << 4 );
					r0[ 3 ] =
					    ( block0->m_redR1 >> 4 ) | ( block0->m_redR2 << 4 );
					r0[ 4 ] =
					    ( block0->m_redR2 >> 4 ) | ( block0->m_redR0 << 4 );
					r0[ 5 ] =
					    ( block0->m_redR0 >> 4 ) | ( block0->m_redR1 << 4 );
					uint8_t r1[ 6 ];
					r1[ 0 ] =
					    ( block1->m_redR4 >> 4 ) | ( block1->m_redR5 << 4 );
					r1[ 1 ] =
					    ( block1->m_redR5 >> 4 ) | ( block1->m_redR3 << 4 );
					r1[ 2 ] =
					    ( block1->m_redR3 >> 4 ) | ( block1->m_redR4 << 4 );
					r1[ 3 ] =
					    ( block1->m_redR1 >> 4 ) | ( block1->m_redR2 << 4 );
					r1[ 4 ] =
					    ( block1->m_redR2 >> 4 ) | ( block1->m_redR0 << 4 );
					r1[ 5 ] =
					    ( block1->m_redR0 >> 4 ) | ( block1->m_redR1 << 4 );
...
void
DDSFile::FlipCompressedImageBC5( ImageData& imageData )
{
	uint32_t numXBlocks = ( imageData.m_width + 3 ) / 4;
	uint32_t numYBlocks = ( imageData.m_height + 3 ) / 4;
	if ( imageData.m_height == 1 ) {}
		...
		for ( uint32_t y = 0; y < ( numYBlocks + 1 ) / 2; y++ )
		{
			...
			for ( uint32_t x = 0; x < numXBlocks; x++ )
			{
				...
				if ( block0 != block1 )
				{
					...
					uint8_t g0[ 6 ];
					g0[ 0 ] =
					    ( block0->m_greenR4 >> 4 ) | ( block0->m_greenR5 << 4 );
					g0[ 1 ] =
					    ( block0->m_greenR5 >> 4 ) | ( block0->m_greenR3 << 4 );
					g0[ 2 ] =
					    ( block0->m_greenR3 >> 4 ) | ( block0->m_greenR4 << 4 );
					g0[ 3 ] =
					    ( block0->m_greenR1 >> 4 ) | ( block0->m_greenR2 << 4 );
					g0[ 4 ] =
					    ( block0->m_greenR2 >> 4 ) | ( block0->m_greenR0 << 4 );
					g0[ 5 ] =
					    ( block0->m_greenR0 >> 4 ) | ( block0->m_greenR1 << 4 );
					uint8_t g1[ 6 ];
					g1[ 0 ] =
					    ( block1->m_greenR4 >> 4 ) | ( block1->m_greenR5 << 4 );
					g1[ 1 ] =
					    ( block1->m_greenR5 >> 4 ) | ( block1->m_greenR3 << 4 );
					g1[ 2 ] =
					    ( block1->m_greenR3 >> 4 ) | ( block1->m_greenR4 << 4 );
					g1[ 3 ] =
					    ( block1->m_greenR1 >> 4 ) | ( block1->m_greenR2 << 4 );
					g1[ 4 ] =
					    ( block1->m_greenR2 >> 4 ) | ( block1->m_greenR0 << 4 );
					g1[ 5 ] =
					    ( block1->m_greenR0 >> 4 ) | ( block1->m_greenR1 << 4 );
