...
static Shader* load_shader_from_file(const std::string& filename)
{
	...
	auto code = read_file_binary(FileSystem::get_shaders_directory() + filename + ".glsl.spv");
	ShaderDesc desc{};
...
static Image* load_image_from_file(const std::string& filename, b32 flip)
{
	u64 size = 0;
	void* data = nullptr;
	ImageDesc image_desc = read_image_data(FileSystem::get_textures_directory() + filename, flip, &size, &data);
