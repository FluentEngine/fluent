...
namespace fluent
{
Texture::Texture( const std::string& directory, const std::string& filename )
    : data( nullptr )
    , filename( filename )
{
...
TextureType to_texture_type( aiTextureType type )
{
    switch ( type )
    {
    case aiTextureType_BASE_COLOR: return TextureType::eBaseColor;
    case aiTextureType_NORMALS: return TextureType::eNormal;
    case aiTextureType_LIGHTMAP:
    case aiTextureType_AMBIENT_OCCLUSION: return TextureType::eAmbientOcclusion;
    case aiTextureType_EMISSIVE: return TextureType::eEmissive;
    case aiTextureType_UNKNOWN: return TextureType::eMetallicRoughness;
    default: FT_ASSERT( false && "Unsupported type" ); return TextureType( -1 );
    }
}

void load_texture_type( Mesh& mesh,
                        Model& model,
                        aiMaterial* mat,
                        aiTextureType type )
{
    for ( u32 i = 0; i < mat->GetTextureCount( type ); i++ )
    {
        ...
        {
            if ( std::strcmp( model.textures[ j ].filename.data(),
                              str.C_Str() ) == 0 )
            {
                   ...
                   const aiScene* scene )
{
    ...
    vertices.resize( vertices.size() +
                     mesh->mNumVertices * model.vertex_stride );
    ...
    {
        // positions
        Vector3 position( mesh->mVertices[ i ].x,
                          mesh->mVertices[ i ].y,
                          mesh->mVertices[ i ].z );
        ...
        if ( b32( vertex_components & VertexComponent::eNormal ) )
        {
            if ( mesh->HasNormals() )
            {
                Vector3 normal( mesh->mNormals[ i ].x,
                                ...
                                mesh->mNormals[ i ].z );
                normal =
                    Matrix3( glm::inverse( glm::transpose( transform ) ) ) *
                    normal;
        ...
        if ( b32( vertex_components & VertexComponent::eTangent ) )
        {
            if ( mesh->mTangents )
            {
                Vector3 tangent( mesh->mTangents[ i ].x,
                                 mesh->mTangents[ i ].y,
                                 mesh->mTangents[ i ].z );
                tangent =
                    Matrix3( glm::inverse( glm::transpose( transform ) ) ) *
    ...
    result.vertex_count =
        ( model.vertices.size() / model.vertex_stride ) - result.first_vertex;
    result.index_count = model.indices.size() - result.first_index;

    return result;
}

void process_mesh_node( Model& model,
                        VertexComponents vertex_components,
                        aiNode* node,
                        const aiScene* scene )
{
    ...
    for ( unsigned int i = 0; i < node->mNumMeshes; ++i )
    {
        ...
        meshes.push_back( process_mesh( model,
                                        Matrix4( t.a1,
                                                 t.b1,
                                                 t.c1,
                                                 t.d1,
...
void fill_vertex_layout( VertexLayout& layout, VertexComponents components )
{
    ...
    if ( b32( components & VertexComponents::eNormal ) )
    {
        ...
        layout.attribute_descs[ attrib_count ].format =
                 ...
                 const std::string& filename )
{
    ...
    FT_ASSERT( scene && ( scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE ) == 0 &&
               scene->mRootNode && "Failed to load model" );
