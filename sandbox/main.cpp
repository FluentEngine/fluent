...
void create_pipeline( VertexLayout& layout )
{
    ...
    Shader* shaders[ 2 ] = {};
    create_shader( GraphicContext::get()->device(),
                   &shader_descs[ 0 ],
                   &shaders[ 0 ] );
    create_shader( GraphicContext::get()->device(),
                   &shader_descs[ 1 ],
    ...
    create_descriptor_set_layout( GraphicContext::get()->device(),
                                  2,
                                  shaders,
                                  &dsl );
    ...
    pipeline_desc.render_pass =
        GraphicContext::get()->swapchain()->render_passes[ 0 ];

    create_graphics_pipeline( GraphicContext::get()->device(),
                              &pipeline_desc,
                              &pipeline );
...
void create_frames()
{
    for ( u32 i = 0; i < GraphicContext::frame_count(); ++i )
    {
        ...
        create_buffer( GraphicContext::get()->device(),
                       &global_ubo_buffer_desc,
                       &frames[ i ].global_buffer );

        global_ubo.projection = camera.get_projection_matrix();
        global_ubo.view       = camera.get_view_matrix();

        ResourceLoader::upload_buffer( frames[ i ].global_buffer,
                                       0,
                                       sizeof( GlobalUbo ),
                                       &global_ubo );

        BufferDesc gpu_objects_data_buffer_desc {};
        gpu_objects_data_buffer_desc.descriptor_type =
            DescriptorType::eStorageBuffer;
        gpu_objects_data_buffer_desc.size =
            MAX_OBJECTS * sizeof( GpuObjectData );
        gpu_objects_data_buffer_desc.memory_usage = MemoryUsage::eCpuToGpu;

        create_buffer( GraphicContext::get()->device(),
                       &gpu_objects_data_buffer_desc,
                       &frames[ i ].objects_buffer );
        ...
        create_descriptor_set( GraphicContext::get()->device(),
                               &desc,
                               &frames[ i ].const_set );
        ...
        update_descriptor_set( GraphicContext::get()->device(),
                               frames[ i ].const_set,
                               2,
                               writes );
    }
...
void create_color_texture( Image** image, const Vector4& color )
{
    ...
    std::vector<Color> image_data( image_desc.width * image_desc.height,
                                   { ( u8 ) ( color.r * 255.0f ),
                                     ( u8 ) ( color.g * 255.0f ),
                                     ( u8 ) ( color.b * 255.0f ),
                                     ( u8 ) ( color.a * 255.0f ) } );
    ResourceLoader::upload_image( *image,
                                  image_data.size() * sizeof( image_data[ 0 ] ),
                                  image_data.data() );
}
...
void create_default_resources()
{
    ...
    create_color_texture( &default_resources.magenta,
                          Vector4( 1.0, 0, 1.0, 1.0 ) );
}
...
Image* upload_mesh_texture( const Model& model,
                            const Mesh& mesh,
                            TextureType type )
{
    ...
    if ( mesh.has_texture( type ) )
    {
        ...
        ResourceLoader::upload_image( loaded_textures.back(),
                                      texture.size,
                                      texture.data );
...
void upload_model( const Model& model, const Matrix4& transform )
{
    ...
    for ( auto& mesh : model.meshes )
    {
        ...
        u64 vertex_upload_size = mesh.vertex_count * model.vertex_stride *
                                 sizeof( model.vertices[ 0 ] );
        ResourceLoader::upload_buffer( geometry_buffer.vertex_buffer,
                                       geometry_buffer.current_vertex_offset,
                                       vertex_upload_size,
                                       model.vertices.data() );

        u64 index_upload_size = mesh.index_count * sizeof( model.indices[ 0 ] );
        ResourceLoader::upload_buffer( geometry_buffer.index_buffer,
                                       geometry_buffer.current_index_offset,
                                       index_upload_size,
                                       model.indices.data() );
        ...
        ImageDescriptor
            image_descriptors[ static_cast<u32>( TextureType::eCount ) ] = {};
...
void on_init()
{
    ...
    VertexComponents components =
        ( VertexComponents::ePosition | VertexComponents::eNormal |
          VertexComponents::eTexcoord | VertexComponents::eTangent );
    ...
    load_model( model,
                components,
                FileSystem::get_models_directory() +
                    "damaged-helmet/DamagedHelmet.gltf" );
    upload_model( model,
                  translate( Matrix4( 1.0 ), Vector3( 0.0, 1.0, 0.0 ) ) );
    model = {};
    load_model( model,
                components,
                FileSystem::get_models_directory() + "plane.gltf" );
...
void on_update( f32 delta_time )
{
    ...
    global_ubo.view = camera.get_view_matrix();
    ResourceLoader::upload_buffer( frame.global_buffer,
                                   0,
                                   sizeof( GlobalUbo ),
                                   &global_ubo );
    ...
    auto* objects_data = ( GpuObjectData* ) ResourceLoader::begin_upload_buffer(
        frame.objects_buffer );
...
int main( int argc, char** argv )
{
    ...
    config.argc        = argc;
    config.argv        = argv;
    config.window_desc = { "sandbox", 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, true };
